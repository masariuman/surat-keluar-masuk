...
@section('title')
  <h1>
    Surat Masuk
    <small>Info Surat Masuk</small>
  </h1>
@endsection

@section('breadcrumb')
  <li><a href="#"><i class="fa fa-dashboard"></i> Surat Masuk</a></li>
  <li class="active">Info Surat Masuk</li>
@endsection
        ...
        <section class="content" >
            ...
            <div class="box">
              ...
              <div style="margin:10px;">
                <a href="/surat-masuk/new" class="btn btn-block btn-primary btn-lg">Tambah Surat Masuk</a>
              </div>
              <hr>
            <div class="box-header">
              <h3 class="box-title">Data Seluruh Surat Masuk</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                  ...
                  @foreach($masuk as $no => $masuk)
                    <tr>
                      ...
                      <td class='text-center'>
                        <button title="LIHAT DATA" id="info" class="btn bg-purple btn-xs" data-photo-id="{{$masuk->id}}" data-toggle="modal" data-target="#modal-default" data-karyawan='{{$masuk}}' data-tgl='{{ Carbon\Carbon::parse($masuk->tanggal)->formatLocalized('%d %B %Y')}}'><i class="fa fa-eye"></i></button>
                        <a title="UBAH DATA" href="{{ url('/surat-masuk/'.$masuk->id.'/edit') }}" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a><br>
                        <button title="HAPUS DATA" class="delete-data btn btn-danger btn-xs" data-photo-id="{{$masuk->id}}"><i class="fa fa-trash"></i></button>
                        <a title="DOWNLOAD DATA" href="{{ url('/tenpureto/dokumen/'.$masuk->dokumen) }}" class="btn btn-success btn-xs"><i class="fa fa-cloud-download"></i></a>
                      </td>
                    </tr>
                  @endforeach
                </tbody>
...
@push('js')
    ...
    <script>
      ...
      function deleteEvent(eventId) {
        ...
        headers: {
          ...
          $.ajax({
            url: "surat-masuk/delete/" + eventId,
            type: "post"
          })
          .done(function(data) {
    ...
    <script>
        $('#modal-default').on('show.bs.modal', function (event) {
          ...
          var alamat_pengirim = button.data('karyawan').alamat_pengirim
          var tanggal = button.data('tgl')
          var nomor = button.data('karyawan').nomor
          var perihal = button.data('karyawan').perihal
          var nomor_petunjuk = button.data('karyawan').nomor_petunjuk
          var nomor_paket = button.data('karyawan').nomor_paket
          ...
          modal.find('.hp').text(nomor_paket)
        })
      </script>
@endpush
